...
def create_advisory(
    ...
    *,
    ...
    db.flush()
    logger.info(
        f"Created Advisory {advisory.id} for session {session_id} "
        f"confidence={confidence:.2f}"
    )
    return advisory
def get_advisory_by_session(
    db: Session,
    session_id: str,
) -> Optional[Advisory]:
    return db.query(Advisory).filter(
        Advisory.session_id == session_id
    ).first()
...
def create_reasoning_step(
    ...
    *,
    ...
    db.flush()
    logger.debug(
        f"ReasoningStep {step_number} persisted: "
        f"session={session_id[:8]} tool={tool_name}"
    )
    return step
...
def set_cached_tool_result(
    ...
    *,
    ...
    logger.debug(
        f"Cached {tool_name} result: key={cache_key} "
    ...
    *,
    ...
    try:
        ...
        logger.info(
            ...
            f"Advisory {advisory.id} created."
        )
