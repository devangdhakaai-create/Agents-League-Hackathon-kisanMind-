...
soil_types = Column(String(50), nullable=False)
...
class Advisory(Base):
    ...
    session_id = Column(
        UUID(as_uuid=False),
        ForeignKey("farmer_sessions.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
    )
    recommendation = Column(Text, nullable=False)
    confidence = Column(Float, nullable=False, defualt=0.0)
    ...
    session = relationship("FarmerSeesions",back_populates="advisory")
    def __repr__(self):
...
class ReasoningStep(Base):
    ...
    __table_args__ = (
        Index("ix_reasoning_steps_session_step", "session_id", "step_number"),
    )
...
class ToolCache(Base):
    ...
    __table_args__ = (
        Index("ix_tool_cache_key", "cache_key"),
    )
